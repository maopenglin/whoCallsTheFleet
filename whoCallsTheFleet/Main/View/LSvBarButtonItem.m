...
+ (instancetype)barButtonItemForHome:(id)target action:(SEL)action
{
    //创建自定义Btn
    UIButton *itemBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    //设置fram
    itemBtn.frame = CGRectMake(0, 0, 15, 15);
    //设置背景图片及渲染方式
    [itemBtn setBackgroundImage:[[UIImage imageNamed:@"homeIcon"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] forState:UIControlStateNormal];
    //绑定回调方法
    [itemBtn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
